...
void display_flush(lv_disp_drv_t *disp, const lv_area_t *area, lv_color_t *color_p)
{
  ...
  lcd.pushPixels((uint16_t *)&color_p->full, w * h, false);
  lcd.endWrite();
...
void lv_button_demo(void)
{
  ...
  lv_obj_add_event_cb(btn1, counter_event_handler, LV_EVENT_PRESSED, NULL);

  lv_obj_set_pos(btn1, 20, 100);  /*Set its position*/
  ...
  lv_obj_add_event_cb(btn2, toggle_event_handler, LV_EVENT_PRESSED, NULL);
  lv_obj_add_flag(btn2, LV_OBJ_FLAG_CHECKABLE);
  lv_obj_set_pos(btn2, 170, 100); /*Set its position*/
